...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
        super(client, handler, {
            command: 'baka',
            description: 'said baka to someone',
            category: 'reactions2',
            usage: `${client.config.prefix}baka [tag/quote users]`
        })
    }
    exec = promisify(exec)
        ...
        M.reply(
            await this.GIFBufferToVideoBuffer(
                await this.client.getBuffer(
                    (
                        await this.client.fetch<{ url: string }>(`https://nekos.life/api/v2/img/baka`)
                    ).url
                )
            ),
            MessageType.video,
            ...
            `*@${M.sender.jid.split('@')[0]} said baka to ${M.mentioned
