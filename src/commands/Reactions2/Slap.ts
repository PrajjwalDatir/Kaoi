...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
        super(client, handler, {
            ...
            category: 'reactions2',
            usage: `${client.config.prefix}slap [tag/quote users]`
        })
    }
        ...
        M.reply(
            await this.client.util.GIFBufferToVideoBuffer(
                await this.client.getBuffer(
                    (
                        await this.client.fetch<{ url: string }>(`https://api.waifu.pics/sfw/slap`)
                    ).url
                )
            ),
            MessageType.video,
            ...
            `*@${M.sender.jid.split('@')[0]} Slapped ${M.mentioned
